...
export default function TrackOrder({ details, awb }) {
  ...
  if (details.current_status) {
    ...
    const foundIdx = steps.findIndex(
      (step) => step.toLowerCase() === status
    );
    if (foundIdx !== -1) {
      currentStepIndex = foundIdx;
    } else if (
      typeof statusMap[details.current_status] === "number"
    ) {
  ...
  const userAddress =
    user.address ||
    order.address ||
    [details.consignee_address, details.consignee_city, details.consignee_pincode]
      .filter(Boolean)
      .join(", ") ||
    "N/A";
  const userEmail =
    ...
    <div className="w-[80vw] mx-auto px-20 sm:px-6 py-5 rounded-2xl shadow-sm border border-[#e6f4f4] bg-white">
            ...
            activity.map((item, i) => (
              <li key={i} className="flex items-center gap-2">
                ...
                <span className="text-gray-700">
                  {item.activity || item.status || item.activity_type || "Status Update"}
                  {item.date_time && (
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {products.length === 0 ? (
            <div className="text-gray-400 text-xs">No product details available.</div>
          ) : (
            products.map((product, i) => (
              ...
              >
                <div>
                  <p className="font-medium text-gray-800">{product.name || product.title || "Product"}</p>
                  {product.description && (
