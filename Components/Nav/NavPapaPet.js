  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (currentScrollY === 0) {
        // At top
        setShowNavbar(true);
        gsap.to(circle.current, { top: "-210%",delay:0.4, duration: 1.2 });
      } else if (currentScrollY > prevScrollY.current) {
        // Scroll down
        setShowNavbar(false);
        gsap.to(circle.current, { top: "-400%", delay:0.4,duration: 1.2 });
        if (isMobile) {
          gsap.to(mobileCircle.current, { y: "-100%", duration: 1.2 });
        }
      } else {
        // Scroll up
        setShowNavbar(true);
        gsap.to(circle.current, { top: "-210%", duration: 0.6 });
        if (isMobile) {
